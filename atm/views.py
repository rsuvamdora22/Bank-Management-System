...
def create_account(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            account.save()
            print(account)
            return redirect('bridge',pk=account.id)
            ...
            if data1:
                ...
                if int(data.account_number) != int(acc_):
                    if int(amount) < balance:
                        ...
                        return redirect('transfersuccess',amt = amount,accn = acc_)
                    else:
                        messages.error(request,'Insufficient Balance')
                    print("one")
                    
                else:
                    messages.error(request,'Unable to transfer to the same account number, check once your account number please!') 
...
def transfersuccess(request,amt,accn):
    return render(request,'transfersuccess.html',{'amt':amt, 'accn':accn})
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if account.otp == otp_entered:
                return redirect('reset_pin',pk = account.id)
            else:
...
def reset_pin(request,pk):
    if request.method == 'POST':
            ...
            if account.check_pin(account.encrypted_pin) != new_pin:
